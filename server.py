...
app = Flask(__name__)
CORS(app, resources={r"/api/*": {"origins": "*"}})  # Adjust the origins as per your requirements
...
@app.route('/api/create_link_token', methods=['POST', 'OPTIONS'])
def create_link_token():
    if request.method == 'OPTIONS':
        # Handle preflight request for CORS
        response = jsonify({'status': 'ok'})  # Provide a simple dummy response
    ...
    try:
        ...
        if PLAID_REDIRECT_URI:
            config.redirect_uri = PLAID_REDIRECT_URI
        ...
        # Make the API call to create the link token
        response = client.link_token_create(config)
        response_json = jsonify(response.to_dict())
